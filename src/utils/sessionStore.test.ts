...
describe('session feature restore', () => {
  it('keeps masks, appearance keyframes, motion settings, and their stage copies', () => {
    const shapeLayers = [
      {
        id: 'mask-shape', shapeType: 'circle' as const, color: '#ffffff', width: 120,
        height: 120, x: 20, y: -10, rotation: 0, opacity: 100,
        position: 'above' as const, maskTarget: { type: 'text' as const, id: 'title' },
        keyframes: [
          {
            id: 'appearance', timeSec: 1, blur: 8, skewX: 12, color: '#ff00aa',
            borderWidth: 6, borderColor: '#00ffff', shadowOpacity: 60,
            shadowBlur: 18, shadowOffsetX: 3, shadowOffsetY: 9,
          },
        ],
      },
    ];
    const textLayers = [
      {
        id: 'title', text: 'Restore me', fontFamily: 'Inter', fontSize: 40,
        fontWeight: '700' as const, fontStyle: 'normal' as const, color: '#ffffff',
        textAlign: 'center' as const, x: 0, y: 0, shadow: true, opacity: 100,
        rotation: 0, position: 'above' as const, letterSpacing: 7, shadowBlur: 14,
        motions: [{ id: 'chars', preset: 'text-rise' as const, startTimeSec: 0, durationSec: 1, textUnit: 'character' as const, textOrder: 'center' as const, staggerSec: 0.04 }],
      },
    ];
    const saved = {
      textLayers,
      ...
      activeStageIndex: 0,
      stages: [{ textLayers, shapeLayers, transitionOut: { type: 'slide-left' as const, durationSec: 0.7, easing: 'ease-in-out' as const } }],
    } as Partial<StudioState>;

    const restored = normalizeRestoredSession(saved);
    expect(restored.shapeLayers?.[0].maskTarget).toEqual({ type: 'text', id: 'title' });
    expect(restored.shapeLayers?.[0].keyframes?.[0]).toMatchObject({ blur: 8, color: '#ff00aa', borderWidth: 6, shadowBlur: 18 });
    expect(restored.textLayers?.[0]).toMatchObject({ letterSpacing: 7, shadowBlur: 14 });
    expect(restored.textLayers?.[0].motions?.[0]).toMatchObject({ preset: 'text-rise', textOrder: 'center', staggerSec: 0.04 });
    expect(restored.stages?.[0].shapeLayers?.[0].maskTarget).toEqual({ type: 'text', id: 'title' });
    expect(restored).toMatchObject({ motionBlurEnabled: true, motionBlurStrength: 72, mockupAnchorX: 0.25 });
  });
